...
class ImplantFieldDropDown extends StatefulWidget {
  final Map typeData;
  const ImplantFieldDropDown({super.key, required this.typeData});

  @override
  State<ImplantFieldDropDown> createState() => _ImplantFieldDropDownState();
}

class _ImplantFieldDropDownState extends State<ImplantFieldDropDown> {
  ...
  Future<void> deleteField() async {
    int response = await dataHelper.deleteData(
        '''DELETE FROM custom_fields_implants WHERE id = $fieldId''');
    if (response > 0) {
  ...
  Future<List> getOptions() async {
    List<Map<String, dynamic>> data = await dataHelper.readData(
        ''' SELECT * FROM field_options_implants WHERE custom_field_id = ${widget.typeData['id']} ''');
    ...
    if (deleteflag > 0) {
      for (TextEditingController option in options) {
        await dataHelper.insertData(
            '''INSERT INTO field_options_implants (custom_field_id,option_value) VALUES ($fieldId,'${option.text}')''');
      }
    }
  }
  ...
  Future<int> deletePreviousOptions() async {
    int deleteFlag = await dataHelper.deleteData(
        '''DELETE FROM field_options_implants WHERE custom_field_id = $fieldId ''');
    return deleteFlag;
  }

  Future<void> modifyFieldName(String newName) async {
    int response = await dataHelper.updateData(
        '''UPDATE custom_fields_implants SET field_name = '$newName' WHERE id = $fieldId  ''');
  }
...
class ImplantFieldText extends StatefulWidget {
  final Map typeData;
  const ImplantFieldText({super.key, required this.typeData});

  @override
  State<ImplantFieldText> createState() => _ImplantFieldTextState();
}

class _ImplantFieldTextState extends State<ImplantFieldText> {
  SqlDb dataHelper = SqlDb();
...
class ImplantFieldDate extends StatefulWidget {
  final Map typeData;
  const ImplantFieldDate({super.key, required this.typeData});

  @override
  State<ImplantFieldDate> createState() => _ImplantFieldDateState();
}

class _ImplantFieldDateState extends State<ImplantFieldDate> {
  SqlDb dataHelper = SqlDb();
      ...
      child: Card(
        child: Row(
          children: [
            ...
            Text(
              AppLocalizations.of(context)!.date_field,
              style: FluentTheme.of(context).typography.caption,
            ),
            SizedBox(
              width: 20,
